...
from mifiel.crypto import AES

class TestAES(BaseTestCase):
  FIXTURES = [
    {
      "algotithm": "AES-128",
  ...
  def test_random_iv_larger_length(self):
    iv = AES.random_iv(24)
    assert len(iv) == 24

  def test_random_iv_with_low_length(self):
    with self.assertRaises(ValueError):
      AES.random_iv(8)

  def test_encrypt(self):
    for test in self.FIXTURES:
      result = AES.encrypt(test['key'], test['dataToEncrypt'], test['iv'])
      assert result == test['encrypted']
